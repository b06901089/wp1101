...
function Post(props) {
  ...
  const getPostDetail = async (pid) => {
    const { data: { message, post }, } = await instance.get('/postDetail', {
      params: { pid, },
    });
    // console.log(`find data in post.js is ${message}`);
    return message, post
  }

  // TODO 5-(2): complete delPost function to delete a post from database
  const delPost = async () => {
    const { data: { message, post }, } = await instance.delete('/post', {
      params: { pid, },
    });
    setTimeout(() => {
      props.navigate(-1)
  ...
  useEffect( async () => {
    var message, post = await getPostDetail(pid);
    setData(post);
  }, [])
