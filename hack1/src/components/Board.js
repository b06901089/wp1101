...
const Board = ({ boardSize, mineNum, backToHome }) => {
    ...
    const updateFlag = (e, x, y) => {
        ...
        let new_board = []
        board.forEach(elem => {new_board.push([...elem])});

        if(!board[x][y].revealed) {
            if(board[x][y].flagged) {
                new_board[x][y].flagged = false;
                setRemainFlagNum(remainFlagNum - 1);
            }
            else {
                new_board[x][y].flagged = true;
                setRemainFlagNum(remainFlagNum + 1);
            }
        }
    ...
    const revealCell = (x, y) => {
        ...
        if(board[x][y].revealed) {
            ...
            console.log('cannot reveal flagged');
        }
        else if(board[x][y].value !== '💣') {
            let temp = revealed(new_board, x, y, nonMineCount);
            setBoard(temp.board);
            ...
            for(let i=0; i<boardSize; i++) {
                for(let j=0; j<boardSize; j++) {
                    if(board[i][j].value === '💣' && board[i][j].flagged === false) {
